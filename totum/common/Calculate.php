...
class Calculate
{
    ...
    static protected $initCodes = [];
    static $paramsCachedMatches = [];
    ...
    static $calcLog = [];
    static $logClassName = "calc";

    static function parseTotumCode($code, $table_name = null)
    {
        $c = [];
        $fixes = [];
        $strings = [];
    ...
    static protected function __compare_normalize($n)
    {
        switch (gettype($n)) {
            case 'NULL':
                return '';
    ...
    static protected function __compare_array_normalize(&$n)
    {
        foreach ($n as &$nItem) {
    ...
    static protected function _compare_n_array($operator, $n, array $n2, $key = null, $isTopLevel = false)
    {
    ...
    static function compare($operator, $n, $n2)
    {
    ...
    static function getDateObject($dateFromParams)
    {
        $date = null;
    ...
    {
        if ($left != 0) {
            $this->__checkDigitParam($left, 'левый элемент', $operator);
        }
        if ($right != 0) {
            $this->__checkDigitParam($right, 'правый элемент', $operator);
        }
        $left = floatval($left);
    ...
    function getReadCodeForLog($code)
    {
    ...
    protected function &AddInLog($title, $type = 'text', $children = [])
    {
        $LogParent = &$this->newLogParent;

        if (static::$logsOn) {

            if (array_key_exists('children', $this->newLogParent)) {
                $this->newLogParent = &$this->newLogParent['children'][];
            }
            $title = $this->getReadCodeForLog($title);

            $children = array_map(function ($child) {
                return ['text' => $this->getReadCodeForLog($child[0]), 'type' => $child[1]];
            },
                $children);

            $this->newLogParent = ['text' => $title . ': ', 'type' => $type, 'children' => $children];
        }
        return $LogParent;
    }
    ...
    {
        if (!$notLoging) {
            $LogParent = &$this->AddInLog($codeName, 'execcode', [[$code, 'code']]);
            $logThisTrait = &$this->newLogParent;
        }
        $mkStart = microtime(1);
        ...
        /*$this->addInLogVar('КОД ' . ($codeName ? '' . $codeName . ': ' : '') . '',
            array_map(function ($item) {
                if (is_array($item)) unset($item['type']);
                return $item;
            },
                $codes));*/
            ...
            if ($k === 'comparison') {
                ...
                switch ($r['type']) {
                    ...
                    case 'func':
                        ...
                        //$this->addInLogVar($func, $r['params']);

                        if (strpos($func, 'ext') === 0) {
                            if (!empty($GLOBALS['CalculateActionExtentions']) && is_object($GLOBALS['CalculateActionExtentions']) && is_callable($GLOBALS['CalculateActionExtentions']->$func)) {
                                $class = $GLOBALS['CalculateActionExtentions'];
                            } elseif (!empty($GLOBALS['CalculateExtentions']) && is_object($GLOBALS['CalculateExtentions']) && is_callable($GLOBALS['CalculateExtentions']->$func)) {
                                $class = $GLOBALS['CalculateExtentions'];
                            } else {
                                throw new errorException('Функция [[' . $func . ']] не найдена');
                            }

                            try {
                                $rTmp = ($class->$func)->call($this, $r['params'], $rTmp);
                            } catch (errorException $e) {
                                if (Auth::isCreator()) {
        ...
        if ($comparison) {
            $r = $this->compare($comparison, $result, $result2);
            $result = $r;
        }

        $diff = round(microtime(1) - $mkStart, 4);
        if ($diff > 0.02) {
            Log::calcs('execSubCode (' . $this->aTable->getTableRow()['name'] . ':' . $this->varName . ') >>>  ' . ($this->getReadCodeForLog($code)) . '(' . $diff . ')');
        }

        if (!$notLoging) {
            $this->AddInLogResult($result, $logThisTrait);
            $this->newLogParent = &$LogParent;
        }
    ...
    function funcGetUsingFields($params)
    {
        ...
        $query = 'select table_name->>\'v\' as table_name, name->>\'v\' as name, version->>\'v\' as version from tables_fields where data->\'v\'->\'code\'->\'==usedFields==\'->\'' . $tableRow['name'] . '\'->>' . Sql::quote($params['field']) . '=\'1\' '
            . 'OR data->\'v\'->\'codeSelect\'->\'==usedFields==\'->\'' . $tableRow['name'] . '\'->>' . Sql::quote($params['field']) . '=\'1\' '
            . 'OR data->\'v\'->\'codeAction\'->\'==usedFields==\'->\'' . $tableRow['name'] . '\'->>' . Sql::quote($params['field']) . '=\'1\'';
    ...
    protected function funcTableLogSelect($params)
    {
        ...
        $data = [];
        if ($params['users']) {
            $slqData = Sql::getAll('select  tableid, cycleid,rowid,field,modify_text,v,action,userid,dt  from _log where userid in ('
                . implode(',',
                    Sql::quote($params['users'],
                        true)) . ') AND dt>=\'' . $date_from . '\' AND dt<\'' . $date_to . '\' order by dt ');
    ...
    protected function funcFileGetContent($params)
    {
        ...
        return File::getContent($params['file']);
    }
            ...
            try {
                ...
                if (!empty($vars['action'])) {
                    ...
                    while ($limit-- > 0) {
                        ...
                        $LogParent = &$this->AddInLog('while iteration' . $whileIterator . ': ');
    ...
    protected function getParam($param, $paramArray)
    {
        ...
        try {
            ...
            switch ($param{0}) {
                case '@':
                    $r = $this->aTable->getSelectByParams(
                    ...
                    if ($param{1} === '#') {
                        ...
                        if (in_array($codeName, $this->fixedCodeNames)) {
                            ...
                            if (!array_key_exists($cacheCodeName, $this->fixedCodeVars)) {
                                ...
                                $this->addInNewLogVar('fixed', '$' . $codeName, $this->fixedCodeVars[$cacheCodeName]);
                            }
                    ...
                    if (array_key_exists($nameVar, $this->whileIterators)) {
                        ...
                        if (preg_match('/^old\./i', $nameVar)) {
                            ...
                            if ($fName = $this->getParam($paramArray['param'], $paramArray)) {
                                if ($selectField = ($this->aTable->getFields()[$fName] ?? null)) {
                                    $paramArray['param'] = '#' . $fName;

                                    $Field = Field::init($selectField, $this->aTable);
    ...
    protected function diffDates($date1, $date2, $unit)
    {
        ...
        switch ($unit) {
            case 'year':
                ...
                $year = $diff->y + $diff->m / 12 + $diff->d / 365;
                return $year;
            case 'month':
                $diff = $date1->diff($date2);
                $month = $diff->m + $diff->y * 12 + $diff->d / 30;
                return $month;
            case 'minute':
    ...
    protected function funcIf($params)
    {
        if ($vars = $this->getParamsArray($params)) {
            ...
            if ($conditionTest) {
                ...
                if (array_key_exists('else', $vars)) return $this->execSubCode($vars['else'], 'else');
                else return null;
            }
    ...
    protected function funcDiffDates($params)
    {
        ...
        $r = $this->diffDates($date1, $date2, $vars['unit'] ?? 'day');
        return $r;
    }
    ...
    {
        return $this->aTable->getTableRow()['name'];
    }

    protected
    function funcNowTableId()
    {
        return $this->aTable->getTableRow()['id'];
    }

    protected
    function funcNowTableUpdatedDt()
    {
        return json_decode($this->aTable->getSavedUpdated(), true)['dt'];
    }
    ...
    {
        if ($this->aTable->getTableRow()['type'] != 'calcs') throw new errorException('[[NowCycleId]] работает только из расчетной таблицы в цикле.');
        return $this->aTable->getCycle()->getId();
    }
        ...
        if ($params = $this->getParamsArray($params)) {
            ...
            foreach (['days' => 'day', 'hours' => 'hour', 'minutes' => 'minute', 'years' => 'year', 'months' => 'month'] as $period => $datePeriodStr) {
                if (!empty($params[$period])) {
                    $this->__checkDigitParam($params[$period], $period, 'dateAdd');
    ...
    function __checkGetDate($dateFromParams, $paramName, $funcName)
    {
        if (empty($dateFromParams) || !($date = static::getDateObject($dateFromParams)))
            throw new errorException('Ошибка формата параметра [[' . $paramName . ' = "' . $dateFromParams . '"]] функции [[' . $funcName . ']] [[' . $this->aTable->getTableRow()['name'] . ']]');
        return $date;
    }
    ...
    protected function funcGetTableSource($params)
    {
        ...
        $params = $this->getParamsArray($params);
        return $this->aTable->getSelectByParams($params
            ,
            "table"
            ,
            $this->row['id'] ?? null
        ...
        switch ($params['key'] ?? 'value') {
            ...
            case 'value':
                ...
                if (!empty($params['direction']) && $params['direction'] == 'desc') {
                    ...
                    if ($isAssoc) asort($params['list'], $flags);
    ...
    {
        $params = $this->getParamsArray($params);
        $this->__checkDigitParam($params['min'], 'min');
        $this->__checkDigitParam($params['max'], 'max');
        $this->__checkDigitParam($params['step'], 'step');
    ...
    function funcSelectRow($params, $codeNameForLog)
    {
        ...
        $row = $this->aTable->getSelectByParams($params,
            'row',
            $this->row['id'] ?? null,
            get_class($this) == Calculate::class
        );
    ...
    function __checkTableIdOrName(&$tableId, $paramName, $funcName = null)
    {
        ...
        if (!ctype_digit((string)$tableId)) $table = Table::getTableRowByName($tableId);
        else  $table = Table::getTableRowById($tableId);
    ...
    protected
    function __checkDigitParam(&$CycleId, $paramName, $funcName = null)
    {
        if (empty($CycleId)) throw new errorException('Не найден параметр [[' . $paramName . ']]');
        if (!is_numeric((string)$CycleId)) {
            throw new errorException('Параметр [[' . $paramName . ']] должен быть числом а не [[' . $CycleId . ']]');
        }
    ...
    function funcReCalculate($params)
    {
        if ($params = $this->getParamsArray($params)) {
            ...
            if ($tableRow['type'] == 'calcs') {
                if (empty($params['cycle']) && $this->aTable->getTableRow()['type'] === 'calcs' && $this->aTable->getTableRow()['tree_node_id'] === $tableRow['tree_node_id']) {
            ...
            } elseif ($tableRow['type'] == 'globcalcs') {

                $Cycle = Cycle::init(0, 0);

                /** @var globcalcsTable $table */
                $table = $Cycle->getTable($tableRow);
                $table->updateFromSource(0);

            } elseif ($tableRow['type'] == 'tmp') {

                if ($this->aTable->getTableRow()['type'] === 'tmp' && $this->aTable->getTableRow()['name'] === $tableRow['name']) {
                    if (empty($params['hash'])) $table = $this->aTable;
                }
                if (empty($table)) $table = tableTypes::getTable($tableRow, $params['hash']);
                ...
                if ($params['where']) {
                    $ids = $table->loadRowsByParams($params['where']);
                    $table->reCalculateFromOvers([
    ...
    {
        $params = $this->getParamsArray($params);
        $ids = $this->select($params, 'treeChildren');
        return $ids;
    }
    ...
    protected function AddInLogResult($result, &$logThisTrait)
    {
        if (static::$logsOn) {
            if (is_array($result)) {
                $logThisTrait['text'] .= 'list из ' . count($result) . ' значений';
                $ires = 10;
                foreach ($result as $k => $v) {
                    if ($k === 0) $val = json_encode([$k => $v], JSON_UNESCAPED_UNICODE | JSON_FORCE_OBJECT);
                    else $val = json_encode([$k => $v], JSON_UNESCAPED_UNICODE);
                    $logThisTrait['children'][] = ['type' => 'list', 'text' => $val];
                    if (--$ires == 0) {
                        $logThisTrait['children'][] = ['type' => 'list', 'text' => '...'];
                        break;
                    }
                }

            } elseif (!is_null($result)) {
                $logThisTrait['text'] .= json_encode($result, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
            }
        }
    ...
    protected function funcBflGet($params)
    {
        $params = $this->getParamsArray($params);
        return Bfl::get($params['category'] ?? '', $params['type'] ?? '');
    }
    ...
    function addInNewLogVar($type, $name, $val, $valIsLog = false)
    {
        if (static::$logsOn) {
            ...
            if ($valIsLog) {
                ...
                $logCode['children'] = [];
                foreach ($val as $k => $v) {
                    if ($k === 0) $logCode['children'][] = ['type' => 'list', 'text' => json_encode([$k => $v],
                        JSON_FORCE_OBJECT | JSON_UNESCAPED_UNICODE)];
                    else $logCode['children'][] = ['type' => 'list', 'text' => json_encode([$k => $v],
                        JSON_UNESCAPED_UNICODE)];
                }
            } else {
                $logCode['text'] = $name . ': ' . json_encode($val, JSON_UNESCAPED_UNICODE);
            }

            if (array_key_exists('children', $this->newLogParent)) {
                $this->newLogParent['children'][] = $logCode;
            } else {
                $this->newLogParent = $logCode;
            }
        }
    }
