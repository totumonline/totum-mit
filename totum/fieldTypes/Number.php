...
class Number extends Field
{
    ...
    protected
    static function modifyNumberValue($modifyVal, $oldValue)
    {

        if (($modifyVal==="" || is_null($modifyVal)) && empty($sign)) return $modifyVal;
        ...
        if(is_object($modifyVal)){
            ...
            if(!is_numeric(strval($diffVal))) throw new errorException('В числовое поле должно идти число, а не [['.strval($diffVal).']]');
            if($percent){
