...
class Select extends Field
{
    ...
    {
        if ($changeFlag === static::CHANGED_FLAGS['changed']) {
    ...
    protected function checkSelectval($channel, $newVal, array $row, array $tbl)
    {
        if (!empty($this->data['checkSelectValues']) && $channel !== 'inner') {
            if (!empty($this->data['withEmptyVal']) && ($newVal === [] || ($newVal ?? '') === '')) {
                return;
            }
            ...
            if (!empty($this->data['multiple'])) {
                foreach ($newVal as $v) {
                    if (!key_exists($v, $list)) {
                        throw new criticalErrorException('This value is not available for entry.');
                    }
                }
            } else {
                if (!key_exists($newVal, $list)) {
                    throw new criticalErrorException('This value is not available for entry.');
                }
            }
        }
