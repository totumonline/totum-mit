...
table tr td.title{font-weight: bold}', 'html' => '{table}'];
    ...
    public function getFullTableData($withRecalculate = true)
    {
        ...
        if (($this->Table->getTableRow()['pagination'] ?? '0/0') == '0/0') {
            $result = array_merge($result, $this->getTableClientData(0, null));
        } else {
            $result = array_merge($result, $this->getTableClientData(0, 0));
        }
        return $result;
    }
    ...
    protected function getTableClientData($page = 0, $onPage = null)
    {
        $data = [];
        $data['rows'] = [];
