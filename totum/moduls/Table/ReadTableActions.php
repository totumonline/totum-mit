...
table tr td.title{font-weight: bold}', 'html' => '{table}'];
    ...
    protected function getPageViewType(): string
    {
        ...
        if ($this->Request->getQueryParams()['iframe'] ?? false) ;
        elseif (($panelViewSettings = ($this->Table->getTableRow()['panels_view'] ?? null))
        ) {
    ...
    public function click()
    {
        ...
        if ($click = is_string($this->post['data']) ? (json_decode($this->post['data'],
                true) ?? []) : $this->post['data']) {
    ...
    protected function getResultTree($filterFunc, $loadingIds)
    {
