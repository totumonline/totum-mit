...
class mockController extends interfaceController
{
    ...
    function __construct($modulName, $inModuleUri, \totum\config\Conf $Config)
    {
        if ($_SERVER['HTTP_HOST'] != "dev3.totum.online") {
...
// Access-Control headers are received during OPTIONS requests
        ...
        Auth::loadAuthUser(static::$user_id, false);
    ...
    static function checkFastMethods()
    {
        if (!empty($_POST['method'])) {
            if (in_array($_POST['method'], array('checkTableIsChanged', 'tmpFileUpload', 'checkForNotifications'))) {
                $_SESSION['userId'] = static::$user_id;
    ...
    function actionAjaxActions()
    {
        ...
        try {
            if ($this->onlyRead && !in_array($method,
                    ['getPanelFormats', 'loadPage', 'linkButtonsClick', 'setCommentsViewed', 'refresh', 'csvExport', 'printTable', 'click', 'getValue', 'loadPreviewHtml', 'notificationUpdate', 'edit', 'checkTableIsChanged', 'getTableData', 'getEditSelect'])) return 'Ваш доступ к этой таблице - только на чтение. Обратитесь к администратору для внесения изменений';
                ...
                case 'getPanelFormats':
                    ...
                    if (!empty($_POST['field']) && !empty($field = $this->Table->getFields()[$_POST['field']])) {
                        ...
                        $result = $clc->getPanelFormat($field['name'],
                            ($field['category'] === 'column' ? $tbl[$_POST['id'] ?? 0] : $tbl['params']),
                            $tbl,
                            $this->Table);
                    ...
                    break;
                case 'getTableData':
                    $table = $this->Table->getTableDataForInterface(false,
                        $this->Table->getTableRow()['pagination'] !== '0/0');
...
//                        , 'notCorrectOrder' => ($table['notCorrectOrder'] ?? false)
                        , 'fields' => $this->fieldsForClient($table['fields'] ?? [])
                        , 'hidden_fields' => \Closure::bind(function () {
